...
This fork includes a **proof-of-concept** daemon that runs Hopper's backend logic in a separate process (intended for WSL2/Linux), exposing a simple **line-delimited JSON-RPC** protocol over TCP.
...
cargo run --bin hopper_daemon -- \
  --listen 127.0.0.1:4732 \
  --data-dir "$HOME/.local/share/hopper-daemon" \
  --token "$TOKEN"
```
