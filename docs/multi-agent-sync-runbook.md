...
Keep Hopper's Agents settings behavior in sync with upstream Codex (`../Codex`) whenever Codex changes:
...
- When users report a mismatch between Hopper settings and Codex runtime behavior
...
## Hopper Files To Update If Upstream Changes
...
- `src-tauri/src/bin/hopper_daemon/rpc/codex.rs`
...
- Keep Hopper scoped to the new key only (no legacy alias read/write).
- Keep Hopper writes aligned with upstream expectations.
...
- Reconcile Hopper guardrails when upstream changes.
...
- Hopper default `agents.max_depth` is `1`.
- Hopper currently enforces a product cap of `12` for `agents.max_threads` and `4` for `agents.max_depth` in UI + backend.
