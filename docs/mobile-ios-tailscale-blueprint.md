# Hopper iOS Remote Blueprint (Tailscale + TCP)

This document is the canonical runbook for iOS remote usage with a desktop-hosted Hopper backend over Tailscale.
...
1. Desktop Hopper hosts the daemon and executes Codex workflows.
2. iOS Hopper connects to the desktop daemon using `remoteBackendHost` + token.
...
- Desktop Hopper is installed and able to run local workspaces.
